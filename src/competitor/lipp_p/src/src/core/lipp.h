...
public:
    ...
    LIPP(double BUILD_LR_REMAIN = 0, bool QUIET = true)
        : BUILD_LR_REMAIN(BUILD_LR_REMAIN), QUIET(QUIET), generator(time(0)) {
      {
    ...
    P at(const T &key, bool skip_existence_check, bool &exist) {
      Node *node = root;
      exist = true;
      ...
      while (true) {
        ...
        if (node_prob_cur == nullptr
            ...
            && node->fixed == 0
            && node->size >= 32
            && num_insert_to_data * 16 >= num_inserts
            && node->size * 4 > node->num_items * 5
            ) {
          num_probability_check++;
          if((generator() & temp) == temp){
            num_probability_trigger++;
            node_prob_prev = parent;
            node_prob_cur = node;
          }
          /*long double p = probability[path_size];
        ...
        if (BITMAP_GET(node->child_bitmap, pos) == 1) {
          ...
          if (skip_existence_check) {
            ...
            if (BITMAP_GET(node->none_bitmap, pos) == 1) {
              ...
              ret = node->items[pos].comp.data.value;
              break;
            }
          }
        }
...
#endif
        scan_and_destory_tree(node_prob_cur, keys, values);
#if COLLECT_TIME
    ...
    void print_depth() const {
      ...
      std::cout << "num_probability_check = " << std::to_string(num_probability_check) << std::endl;
      std::cout << "num_probability_trigger = " << std::to_string(num_probability_trigger) << std::endl;
      std::cout << "num_insert_to_data * 5 >= num_inserts && (generator() & 0xff) == 0xff" << std::endl;
    }
...
private:
    ...
    long long num_probability_check = 0;
    long long num_probability_trigger = 0;
    ...
    Node *insert_tree(Node *_node, const T &key, const P &value, bool *ok = nullptr) {
      ...
      for (Node *node = _node;;) {
        ...
        if (BITMAP_GET(node->none_bitmap, pos) == 1) {
          ...
          node->items[pos].comp.child = build_tree_two(key, value, node->items[pos].comp.data.key,
      ...
      for (int i = 0; i < path_size; i++) {
        ...
        const int num_inserts = node->num_inserts;
        const int num_insert_to_data = node->num_insert_to_data;
        /*const bool need_rebuild = node->fixed == 0 && node->size >= node->build_size * 2 && node->size >= 64 &&
                                  num_insert_to_data * 10 >= num_inserts;*/
        const bool need_rebuild = node->fixed == 0 && node->size >= node->num_items * 2 && node->size >= 128 &&
                                  num_insert_to_data * 10 >= num_inserts;
