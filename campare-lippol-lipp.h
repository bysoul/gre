    ...
    bool at(const T &key, P &value) {
      ...
      if (++skip_counter== skip_length) {
        for (int i = 0; i < path_size; i++) {
          path[i]->node_counter_map[omp_get_thread_num()].field++;
        }
...
private:
    ...
    struct Node {
        ...
        LinearModel<T> model;
        Item *items;
        //static thread_local std::unordered_map<Node*, int> node_counter_map;
        std::array<AlignedStruct, 64> node_counter_map;
    };
    ...
    std::atomic<int> num_rebuild = 0;

    int skip_length = 10;
    //static thread_local std::unordered_map<LIPP<T,P,USE_FMCD>*, int> skip_counter_map;
    //std::array<int, 1024> skip_counter_map {};
    /*thread_local static  int skip_counter;
    int &get_skip_counter() {

      return skip_counter;
    }*/


    std::bernoulli_distribution pq_distribution{0.00000001};

    std::minstd_rand &get_generator() {
